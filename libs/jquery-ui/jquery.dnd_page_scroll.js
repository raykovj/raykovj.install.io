(function($){
	...
	$.fn.dndPageScroll = function(options) {
		...
		if (!top_el.length)
			top_el = $('<div id="top_scroll_page">&nbsp;</div>').appendTo($("#canvas"));
		
		var bottom_el = $('#'+options.bottomId);
		if (!bottom_el.length)
			bottom_el = $('<div id="bottom_scroll_page">&nbsp;</div>').appendTo($("#canvas"));
		...
		both_el.on('dragover', function(e) {
			...
			//var current = (direction==-1)?scrollTop:$(document).height()-(scrollTop+$("#canvas").height());
			var current = (direction==-1)?scrollTop:$('#container').height()-(scrollTop+height);
